...
public class DisplayMainSqlAction extends NCAction {
	...
	public DisplayMainSqlAction(){
		this.setBtnName("查看主表取数规则");
	}
	...
	public void doAction(ActionEvent paramActionEvent) throws Exception {
		...
		pk_def=(String) this.getBilllist().getBillCardPanel().getBodyValueAt(row[0], "smartdef1");
		if (pk_def==""){
			MessageDialog.showErrorDlg(this.billlist, "错误", "语义模型主表参照值为空，不能显示取数规则！");
			return;
		}
